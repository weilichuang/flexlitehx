...
class DxrDrawer
{
	...
	*/
    public function drawDisplayObject(dp : DisplayObject, dxrData : DxrData, frame : Int) : Void
    {
        ...
        var tempBmData : BitmapData = new BitmapData(Std.int(dpRect.width + offsetX * 2), Std.int(dpRect.height + offsetY * 2), true, 0);
        ...
        var frameData : BitmapData = new BitmapData(Std.int(colorRect.width), Std.int(colorRect.height), true, 0);
	...
	* 绘制一个显示对象的当前外观，存储其一帧位图信息到dxrData内
	*/
    public function drawWithoutFilter(dp : DisplayObject, dxrData : DxrData, frame : Int) : Void
    {
        var dpRect : Rectangle = dp.getBounds(dp);
        ...
        var frameData : BitmapData = new BitmapData(Std.int(dpRect.width), Std.int(dpRect.height), true, 0);
        var ct : ColorTransform = drawColorTransfrom(dp);
	...
	*/
    public function drawColorTransfrom(dp : DisplayObject) : ColorTransform
    {
	...
	*/
    public function getColorRect(bitmapData : BitmapData) : Rectangle
    {
