...
import flexlite.components.supportclasses.Skin;
...
class VectorSkin extends Skin
{
	...
	* @param height 渐变的高度。
	*/
    private function verticalGradientMatrix(x : Float, y : Float,
            width : Float,
            height : Float) : Matrix
    {
        tempMatrix.createGradientBox(width, height, 0.5 * Math.PI, x, y);
        return tempMatrix;
    }
    /**
	* 创建水平渐变矩阵。
	...
	* @param height 渐变的高度。
	*/
    private function horizontalGradientMatrix(x : Float, y : Float,
            width : Float,
            height : Float) : Matrix
    {
	...
	* 文本投影滤镜
	*/
    public static var textOverFilter : Array<BitmapFilter> = [new DropShadowFilter(1, 270, 0, 0.3, 1, 1)];
    /**
	* 边框线条颜色
	*/
    public static var borderColors : Array<UInt> = [0xD4D4D4, 0x518CC6, 0x686868];
    /**
	* 底线颜色
	*/
    public static var bottomLineColors : Array<UInt> = [0xBCBCBC, 0x2A65A0, 0x656565];
    /**
	* 边框圆角
	*/
    public static var cornerRadius : Float = 3;
    /**
	* 填充颜色 
	*/
    public static var fillColors : Array<UInt> = [0xFAFAFA, 0xEAEAEA, 0x589ADB, 0x3173B4, 0x777777, 0x9B9B9B];
    /**
	* 主题颜色，应用到文本或者图标上
	*/
    public static var themeColors : Array<UInt> = [0x333333, 0xFFFFFF];
    /**
	* 绘制当前的视图状态
	*/
    private function drawCurrentState(x : Float, y : Float, w : Float, h : Float, borderColor : UInt,
										bottomLineColor : UInt, fillColors : Dynamic, cornerRadius : Dynamic = null, g : Graphics = null) : Void
    {
        var crr : Dynamic = cornerRadius;
        var crr1 : Dynamic;
        if (cornerRadius == null || Std.is(cornerRadius, Float)) 
        {
            crr1 = cornerRadius == null ? 0 : cast cornerRadius - 1;
            if (crr1 < 0) 
                crr1 = 0;
            crr1 = {
                        tl : crr1,
        ...
        {
            crr1 = {
                        tl : Math.max(0, crr.tl - 1),
                        tr : Math.max(0, crr.tr - 1),
                        bl : Math.max(0, crr.bl - 1),
                        br : Math.max(0, crr.br - 1),
                    };
        }  
	...
	*/
    private function drawLine(startX : Float, startY : Float, endX : Float, endY : Float, color : Int, g : Graphics = null) : Void
    {
        if (g == null) 
            g = graphics;
        g.lineStyle(1, color);
	...
	* @param g 要绘制到的目标Graphics，留空则绘制到自身。
	*/
    private function drawRoundRect(
            x : Float, y : Float, width : Float, height : Float,
            cornerRadius : Dynamic = null,
            color : Dynamic = null,
            alpha : Dynamic = null,
            gradientMatrix : Matrix = null,
            gradientType : GradientType = null,
            gradientRatios : Array<Dynamic> = null,
            hole : Dynamic = null, g : Graphics = null) : Void
    {
        if (g == null) 
            g = graphics;
        ...
        if (color != null) 
        {
            if (Std.is(color, Int)) 
            {
                g.beginFill(cast color, Std.parseFloat(alpha));
            }
            else if (Std.is(color, Array)) 
            {
                var alphas : Array<Dynamic> = Std.is(alpha, Array) ? cast alpha : [alpha, alpha];
                
                if (gradientRatios == null) 
                    gradientRatios = [0, 0xFF];
                
                g.beginGradientFill(gradientType, cast color, alphas, gradientRatios, gradientMatrix);
            }
        }
        
        var ellipseSize : Float;
        
        if (cornerRadius == null) 
        {
            g.drawRect(x, y, width, height);
        }
        else if (Std.is(cornerRadius, Float)) 
        {
            ellipseSize = Std.parseFloat(cornerRadius) * 2;
        ...
        if (hole != null) 
        {
            var holeR : Dynamic = hole.r;
            if (Std.is(holeR, Float)) 
            {
                ellipseSize = Std.parseFloat(holeR) * 2;
