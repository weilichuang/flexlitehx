...
* @author weilichuang
*/
class DropDownListSkin extends VectorSkin
{
    public function new()
    {
        super();
	...
	* 滚动条皮肤
	*/
    public var scrollerSkinName : Dynamic;
	...
	*/
    ...
    {
        var state : String = (Lib.as(openButton.skin, IStateClient)).currentState;
        if (state == "over" || state == "down") 
        {
            labelDisplay.textColor = VectorSkin.themeColors[1];
            labelDisplay.applyTextFormatNow();
            labelDisplay.filters = VectorSkin.textOverFilter;
        }
        else 
        {
            labelDisplay.textColor = VectorSkin.themeColors[0];
            labelDisplay.applyTextFormatNow();
