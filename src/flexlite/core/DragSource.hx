...
class DragSource
{
    public var formats(get, never) : Array<Dynamic>;
	...
	*/
    ...
    private var formatHandlers : Dynamic = { };
    
    private var _formats : Array<Dynamic> = [];
    /**
	* 包含拖动数据的格式，以字符串 Array 的形式表示。
	* 使用 addData() 或 addHandler() 方法设置此属性。默认值取决于添加到 DragSource 对象的数据。
	*/
    private function get_formats() : Array<Dynamic>
    {
        return _formats;
    }
    /**
	...
	*/
    public function addData(data : Dynamic, format : String) : Void
    {
        _formats.push(format);
        
        Reflect.setField(dataHolder, format, data);
    }
	...
	*/
    public function addHandler(handler : Void->Void,format : String) : Void
    {
        _formats.push(format);
        
        Reflect.setField(formatHandlers, format, handler);
    }
    /**
	...
	* @param format 描述此数据格式的字符串。
	*/
    public function dataForFormat(format : String) : Dynamic
    {
        var data : Dynamic = Reflect.field(dataHolder, format);
        if (data != null) 
            return data;
        
        if (Reflect.hasField(formatHandlers, format)) 
            return Reflect.field(formatHandlers, format)();
